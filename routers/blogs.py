...
async def create_blog(blog: Blog, bm: BlogManager = Depends(get_blog_manager), 
    get_current_user: User = Depends(get_current_user)):    
    return await bm.add_blog(blog, get_current_user) 


@router.delete('/{blog_id}', tags=['Blogs'])
async def delete_blog(blog_id: int, bm: BlogManager = Depends(get_blog_manager), 
    get_current_user: User = Depends(get_current_user)) :
    return await bm.remove_blog_by_id(blog_id)


@router.get('/{blog_id}', tags=['Blogs'])
async def get_blog_by_id(blog_id: int, bm: BlogManager = Depends(get_blog_manager), 
    get_current_user: User = Depends(get_current_user)) -> Optional[Blog]:
